...
describe "Books API" do
  ...
  it 'can sell a book' do
    id = create(:book, number_sold: 4).id

    patch "/api/v1/books/#{id}/sell"
    book = Book.find_by(id: id)

    expect(response).to be_successful
    expect(book.number_sold).to eq(5)
  end

  it 'can get books of a given reading level' do
    books_2 = create_list(:book, 4, reading_level: 2)
    books_5 = create_list(:book, 3, reading_level: 5)

    get  "/api/v1/books?reading_level=2"
